  ...
  return (
    <div>
      <h2 id="position-heading" data-cy="PositionHeading">
        <Translate contentKey="jhipsterRosterApplication5App.position.home.title">Positions</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipsterRosterApplication5App.position.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/position/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipsterRosterApplication5App.position.home.createLabel">Create new Position</Translate>
          </Link>
        </div>
      </h2>
        ...
        {positionList && positionList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhipsterRosterApplication5App.position.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('key')}>
                  <Translate contentKey="jhipsterRosterApplication5App.position.key">Key</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('key')} />
                </th>
                <th className="hand" onClick={sort('leadership')}>
                  <Translate contentKey="jhipsterRosterApplication5App.position.leadership">Leadership</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('leadership')} />
                </th>
                <th>
                  <Translate contentKey="jhipsterRosterApplication5App.position.department">Department</Translate>{' '}
            ...
            <tbody>
              {positionList.map((position, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{position.department ? <Link to={`/department/${position.department.id}`}>{position.department.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterRosterApplication5App.position.home.notFound">No Positions found</Translate>
            </div>
          )
        )}
      </div>
