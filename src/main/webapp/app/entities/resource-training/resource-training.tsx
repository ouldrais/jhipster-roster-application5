  ...
  return (
    <div>
      <h2 id="resource-training-heading" data-cy="ResourceTrainingHeading">
        <Translate contentKey="jhipsterRosterApplication5App.resourceTraining.home.title">Resource Trainings</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipsterRosterApplication5App.resourceTraining.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/resource-training/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            ...
            &nbsp;
            <Translate contentKey="jhipsterRosterApplication5App.resourceTraining.home.createLabel">Create new Resource Training</Translate>
          </Link>
        </div>
      </h2>
        ...
        {resourceTrainingList && resourceTrainingList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhipsterRosterApplication5App.resourceTraining.id">ID</Translate>{' '}
                ...
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {resourceTraining.resource ? (
                      <Link to={`/resource/${resourceTraining.resource.id}`}>{resourceTraining.resource.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td>
                    {resourceTraining.training ? (
                      <Link to={`/training/${resourceTraining.training.id}`}>{resourceTraining.training.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterRosterApplication5App.resourceTraining.home.notFound">No Resource Trainings found</Translate>
            </div>
          )
        )}
      </div>
